...
export async function POST(request: NextRequest) {
  try {
    ...
    if (task_names && Array.isArray(task_names) && task_names.length > 0) {
      ...
      console.log(`[SQL Access Clear] Found ${tasks.length} tasks to delete from owners:`,
      ...
      for (const task of tasks) {
        try {
          ...
          deletedCount++;
          console.log(`[SQL Access Clear] Deleted task: ${task.TASK_NAME}`);
        } catch (error) {
          console.error(`[SQL Access Clear] Failed to delete task ${task.TASK_NAME}:`, error);
          // 개별 작업 삭제 실패는 무시하고 계속 진행
        }
      }
    }
