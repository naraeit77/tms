/**
 * LLM Execution Plan Analysis API
...
import { AnalyzeExecutionPlanUseCase } from '@/application/llm-analysis'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.planText) {
      return NextResponse.json(
        { success: false, error: { code: 'INVALID_REQUEST', message: 'planText is required' } },
        { status: 400 }
      )
    }
    ...
    const useCase = new AnalyzeExecutionPlanUseCase(adapter)
    ...
    console.error('LLM execution plan analysis error:', error)
