...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sqlId) {
      // 특정 SQL의 실행계획 조회
      // Oracle SQL_ID는 항상 소문자로 저장되므로 소문자로 변환
      ...
      if (!result.rows || result.rows.length === 0) {
        // V$SQL_PLAN에 데이터가 없는 경우, V$SQL에서 plan_hash_value만 조회
        const fallbackQuery = `
          SELECT
            sql_id,
            plan_hash_value,
        ...
        if (!fallbackResult.rows || fallbackResult.rows.length === 0) {
          return NextResponse.json({
            ...
            message: `SQL_ID '${normalizedSqlId}'를 V$SQL 또는 V$SQL_PLAN에서 찾을 수 없습니다. SQL이 메모리에서 제거되었거나 존재하지 않는 SQL_ID입니다.`,
            data: [],
            count: 0,
          });
        }
        ...
        return NextResponse.json({
          success: true,
          warning: 'V$SQL_PLAN에 실행계획 정보가 없습니다. SQL이 메모리에서 제거되었을 수 있습니다.',
          data: [{
            ...
            plan_text: '※ 실행계획을 조회할 수 없습니다.\n\n이유: SQL이 오래되어 메모리(V$SQL_PLAN)에서 제거되었거나,\n실행계획 정보가 생성되지 않았습니다.\n\n대안:\n1. SQL을 다시 실행하여 메모리에 로드\n2. AWR 히스토리에서 과거 실행계획 조회 (DBA_HIST_SQL_PLAN)\n3. SQL Tuning Advisor 사용',
      ...
      const query = `
        SELECT DISTINCT
          ...
          s.plan_hash_value,
          s.executions,
          s.elapsed_time / DECODE(s.executions, 0, 1, s.executions) / 1000 as avg_elapsed_ms,
          s.cpu_time / DECODE(s.executions, 0, 1, s.executions) / 1000 as avg_cpu_ms,
          s.buffer_gets / DECODE(s.executions, 0, 1, s.executions) as avg_buffer_gets,
          s.disk_reads / DECODE(s.executions, 0, 1, s.executions) as avg_disk_reads,
          s.rows_processed / DECODE(s.executions, 0, 1, s.executions) as avg_rows,
          s.first_load_time,
          s.last_active_time
        FROM
          v$sql s
